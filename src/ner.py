...
from src.data_processing import (create_embedding_matrix, create_lookup_tables, text_to_indices, train_word2vec,)
    ...
    with open(infile, "r", encoding="utf-8") as f:
        ...
        for row_num, row in enumerate(reader, start=1):
            ...
            if len(tokens) != len(ner_tags):
                print(
                    f"[WARNING] Fila {row_num} ignorada: "
                    f"{len(tokens)} tokens vs {len(ner_tags)} etiquetas"
                )
                continue
...
def load_processed_ner_data_with_embeddings(infile: str, min_freq: int = 2, max_len: int = 100, embedding_dim: int = 100,):
    ...
    return (np.array(X, dtype=np.int64), np.array(y, dtype=np.int64), word2idx, idx2word, label2idx, idx2label,embedding_matrix,)
...
class BiLSTMNER(nn.Module):
    def __init__(self, vocab_size: int, embedding_dim: int, hidden_dim: int, output_dim: int, padding_idx: int, embedding_matrix: np.ndarray = None,dropout: float = 0.3,):
        super().__init__()
...
def train_one_epoch(model: nn.Module, dataloader: DataLoader, optimizer: torch.optim.Optimizer, criterion: nn.Module, device: torch.device,pad_idx: int):
    model.train()
    total_loss = 0.0
    total_acc = 0.0
...
def train_ner_model(csv_path: str = "data/ner_dataset.csv", min_freq: int = 2,max_len: int = 100,
    ...
    """
    Lee ner_dataset.csv, prepara datos y entrena un modelo NER.
    """
    ...
    model = BiLSTMNER(vocab_size=len(word2idx), embedding_dim=embedding_dim, hidden_dim=hidden_dim, output_dim=len(label2idx), padding_idx=pad_word_idx, embedding_matrix=embedding_matrix,).to(device)
    ...
    for epoch in range(epochs):
        train_loss, train_acc = train_one_epoch(
            model, train_loader, optimizer, criterion, device, pad_label_idx
        )
        val_loss, val_acc = evaluate(
            model, val_loader, criterion, device, pad_label_idx
        )
...
def predict_sentence(
    ...
    indices = text_to_indices(sentence_tokens, word2idx, max_len=max_len)
    ...
    valid_len = min(len(sentence_tokens), max_len)
    pred_labels = [idx2label[int(preds[i])] for i in range(valid_len)]

    return list(zip(sentence_tokens[:valid_len], pred_labels))


def main():
    model, history, word2idx, idx2word, label2idx, idx2label = train_ner_model(
    ...
    print("\nEntrenamiento NER terminado.")
    sample_tokens = ["sam", "worthington", "is", "in", "avatar"]
    preds = predict_sentence(sample_tokens, model, word2idx, idx2label,max_len=100)

    print("\nPredicción de ejemplo:")
    for token, tag in preds:
